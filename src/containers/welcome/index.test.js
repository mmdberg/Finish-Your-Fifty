...
describe('Welcome', () => {
  ...
  describe('signup', () => {
    ...
    it('should clear inputs and set error state if email exists in database', async () => {
      const expected = {
        userName: '',
        email: '',
        password: '',
    ...
    it('should call addUser if email does not exist', async () => {
      await wrapper.instance().signUp();
      expect(api.addUser).toHaveBeenCalled();
    });

    it('should call captureUser with the right params if user signs up', async () => {
      await wrapper.instance().signUp();
      expect(mockCaptureUser).toHaveBeenCalled();
      //add right params
    });

    it('should reset state if user signs up', async () => {
      const expected = {
        userName: '',
        email: '',
        password: '',
